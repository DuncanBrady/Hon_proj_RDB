...
# ----------------------------
# Data loading helper (moved from import time to function to allow reuse)
# ----------------------------
...
# ----------------------------
@torch.no_grad()
def reconstruct_full(model: nn.Module, device: torch.device, gene_tensor: torch.Tensor, batch_size: int = 256):
    model.eval()
...
# Training
# ----------------------------
def train_vq_autodecoder(dataloader, device, gene_tensor: torch.Tensor, epochs=800, log_filename="vq_autodecoder_metrics.txt"):
    G, C = gene_tensor.shape
    # Model
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 100 == 0:
            ...
            with torch.no_grad():
                ...
                full_recon = reconstruct_full(model, device, gene_tensor, batch_size=256)
...
if __name__ == "__main__":
    # Default standalone training preserves previous behaviour using env path
    path = os.path.expanduser("/g/data/yr31/mp8713/")
    data_file = os.path.join(path, "sct_matrix_transposed.npz")
    gene_tensor = load_expression_tensor(data_file)
    ...
    model = train_vq_autodecoder(dl, device, gene_tensor, epochs=4000, log_filename="vqvae2_metrics.txt")
    ...
    # Recon + save
    recon = reconstruct_full(model, device, gene_tensor, batch_size=256)
